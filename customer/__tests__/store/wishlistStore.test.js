...
describe('fetchWishlist', () => {
    test('fetches wishlist items successfully', async () => {
        ...
        expect(useWishlistStore.getState().items)
            .toEqual(mockItems);

        expect(useWishlistStore.getState().isLoading)
            .toBe(false);
    ...
    test('sets isLoading during fetch', async () => {
        ...
        wishlistApi.getWishlist.mockReturnValueOnce(
            new Promise((resolve) => {
                resolvePromise = resolve;
            })
        );

        const promise = useWishlistStore
            .getState()
            .fetchWishlist();
    ...
    test('handles 401 error silently', async () => {
        wishlistApi.getWishlist.mockRejectedValueOnce({
            status: 401,
        });
        ...
        expect(useWishlistStore.getState().error)
            .toBeNull();
...
describe('addToWishlist', () => {
    test('calls API and queues wishlist refresh', async () => {
        ...
        expect(wishlistApi.addToWishlist)
            .toHaveBeenCalledWith('p1', 'v1');

        jest.runAllTimers();

        expect(wishlistApi.getWishlist)
            .toHaveBeenCalled();
    });
    ...
    test('re-throws error on failure', async () => {
        wishlistApi.addToWishlist.mockRejectedValueOnce(
            new Error('Failed')
        );
...
describe('removeItem', () => {
    test('calls API and queues wishlist refresh', async () => {
        ...
        expect(wishlistApi.removeFromWishlist)
    ...
    test('re-throws error on failure', async () => {
        ...
        wishlistApi.removeFromWishlist.mockRejectedValueOnce(
            new Error('Failed')
        );
...
describe('moveToCart', () => {
    ...
    test('re-throws error on failure', async () => {
        ...
        wishlistApi.moveToCart.mockRejectedValueOnce(
            new Error('Failed')
        );
